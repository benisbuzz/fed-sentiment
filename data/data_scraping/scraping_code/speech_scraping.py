...
def find_speeches_by_year(host, this_url, print_test=False):
    ...
    conn = HTTPSConnection(host=host, context=ssl_context)
    ...
    if resp.status != 200:
        ...
        for link in event_list.find_all(
            "a", href=True, class_=lambda x: x != "watchLive"
        ):
...
def create_speech_df(host, annual_htm_list):
    ...
    for item in annual_htm_list:
        date_lst, speaker_lst, title_lst, link_lst = find_speeches_by_year(
            host, item, print_test=False
        )
...
def get_one_doc(host, this_url):
    ...
    OUTPUTS:
        list  containing the split text of the speech
    ...
    t = re.sub("  ", " ", t)
    t = re.sub(r"(?<=[.,;])(?=[^\s])", r" ", t)
    sent_tokens = sent_tokenize(joined_doc.replace("\n", ""))
