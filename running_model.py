...
async def _scrape_and_call(dates: list[str], base_url: str, is_pc: bool, model: str, instructions: list[str], prompt_name: str):
    ...
    for date in dates:
        ...
        if is_pc:
            ...
            speaker_split = tkn.get_speaker_text(raw_release, get_txt_list("./data/scraping/fed_chairs.txt"))
        ...
        labels = [_find_label(sentiment[1]) if multiple_instructions is True else _find_label(sentiment) for sentiment in sentiments]
        reasons = [sentiment[0] if multiple_instructions is True else sentiment for sentiment in sentiments]
...
async def test_model(
    ...
    if pc_dates:
        ...
        await _scrape_and_call(pc_dates, base_url, True, model, instructions, prompt_name)
    if sp_dates:
        base_url = "https://www.federalreserve.gov/newsevents/speech/files/"
        await _scrape_and_call(sp_dates, base_url, False, model, instructions, prompt_name)
